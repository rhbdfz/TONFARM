...
class TonConnectService {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _tonConnect.onStatusChange((event) {
        final connected = _tonConnect.account != null;
      ...
      _isInitialized = true;
      debugPrint('TON Connect инициализирован');
    } catch (e) {
  ...
  Future<List<WalletApp>> getAvailableWallets() async {
    try {
      await initialize();
      final wallets = await _tonConnect.getWallets();
      return wallets;
    } catch (e) {
      _errorController.add('Не удалось получить кошельки: $e');
      return [];
    }
  }
  ...
  Future<bool> connectWallet([WalletApp? wallet]) async {
    try {
      await initialize();
      final wallets = wallet != null ? [wallet] : await _tonConnect.getWallets();
      if (wallets.isEmpty) {
  ...
  Future<void> disconnect() async {
    try {
      ...
      _errorController.add('Ошибка отключения: $e');
    }
  }
